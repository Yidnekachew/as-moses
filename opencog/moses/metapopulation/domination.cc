...
void metapopulation::remove_dominated(scored_program_set& bcs, unsigned jobs)
{
    ...
    scored_program_ptr_vec bcv = random_access_view(bcs);
    scored_program_ptr_vec res = get_nondominated_rec(bcv, jobs);
    // get the dominated by set difference
    boost::sort(bcv); boost::sort(res);
    scored_program_ptr_vec dif = set_difference(bcv, res);
    // remove the dominated ones
    for (const scored_program* cnd_ptr : dif)
        bcs.erase(*cnd_ptr);
}
...
metapopulation::get_nondominated_iter(const scored_program_set& bcs)
{
    ...
    return scored_program_set(mcl.begin(), mcl.end());
}

typedef std::pair<scored_program_set,
                  scored_program_set> scored_program_set_pair;

typedef std::pair<scored_program_set, scored_program_set> scored_program_set_pair;
...
metapopulation::get_nondominated_rec(const scored_program_ptr_vec& bcv,
                     unsigned jobs)
{
    ///////////////
    // base case //
...
scored_program_set
metapopulation::to_set(const scored_program_ptr_vec& bcv)
{
    scored_program_set res;
    for (const scored_program* cnd : bcv)
        res.insert(*cnd);
    return res;
}

scored_program_set_pair
metapopulation::get_nondominated_disjoint(const scored_program_set& bcs1,
...
scored_program_ptr_vec_pair
metapopulation::get_nondominated_disjoint_rec(const scored_program_ptr_vec& bcv1,
                              ...
                              unsigned jobs)
{
    ...
    if (bcv1.empty() || bcv2.empty())
        ...
        scored_program_ptr_vec bcv_res1, bcv_res2;
        scored_program_ptr_vec_cit it1 = bcv1.begin(),
            it2 = bcv2.begin();
